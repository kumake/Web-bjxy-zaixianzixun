...
<table width="98%" border="0" align="center" cellpadding="5" cellspacing="1">
  <tr>
	...
	<a href="book_revert.asp?id=<%=rs("id")%>&page=<%=page_num%>" >回复</a>
	&nbsp;
	<%
		if rs("istop")=0 then
	%>
	...
	<%end if%>
	<a href="book_del.asp?id=<%=rs("id")%>&page=<%=page_num%>" onClick="return confirm('是否确认删除？');">删除</a>&nbsp;	</div></td>
  </tr>
</table>
<%end if%>
<table width="98%" border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#000000" >
...
%>
当前第<%=page_num%>/<%=rs.pagecount%> 共有<%=rs.pagecount%>页&nbsp;<%=rs.recordcount%>条记录&nbsp; 每页显示<%=page_size%>条信息&nbsp; &nbsp;
<a href="?page=1">首  页</a>&nbsp;
<%
	if page_num<=1 then 
		response.Write("上一页")
...
<%
	if cint(page_num)>=cint(rs.pagecount) then
		response.Write("下一页")
	else	
%>
<a href="?page=<%=page_num+1%>">下一页</a>
<%end if%>
&nbsp;<a href="?page=<%=rs.pagecount%>">末页  </a>
<select name="pagelist" onChange="MM_jumpMenu('self',this,0)">                           
   <% for i=1 to rs.pagecount %>
   <option VALUE="?page=<%=i%>" <% if cint(page_num)=i then%>selected="selected"<%end if%>>第<%=i%>页</option>
   <%next%>
</select>
<%	
	rs.close
